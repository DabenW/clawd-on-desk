...
// Port range for CatPaw terminal-focus extension instances.
...
function tryListen(port, maxPort) {
  if (port > maxPort) {
    console.log("CatPaw terminal-focus: all ports in use, HTTP server disabled");
    return;
  }
  ...
  server.listen(port, "127.0.0.1", () => {
    boundPort = port;
    console.log(`CatPaw terminal-focus: listening on 127.0.0.1:${port}`);
  });
}
...
function activate(context) {
  ...
  // vscode://catpaw.catpaw-terminal-focus?pids=1234,5678
